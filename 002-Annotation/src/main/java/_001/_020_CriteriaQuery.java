...
- Criteria Query: Instead of specifying all the criteria into your HQL which makes it messier, we can use criteria objects to specify the criteria. 
Criteria API allows you to build up a criteria query object where you can apply filtration rules and logical conditions.

2. The Hibernate Session interface provides createCriteria() method which can be used to create a Criteria object that returns instances of the 
persistence class when your application executes a criteria query. You can use add() method available for Criteria object to add restriction for a 
criteria query.

3. Projections: The Criteria API provides the org.hibernate.criterion.Projections class which can be used to get average, maximum or minimum of the 
property values. The Projections class is similar to the Restrictions class in that it provides several static factory methods for obtaining Projection 
instances.
...
4. Criteria Interface provides the following methods to restrict the records fetched from DB:
...
5. Class Restriction provides built-in criterion via static factory methods. Important methods of the Restriction class are:
...
public class _020_CriteriaQuery{
	...
	public static void main(String[] args) throws IOException {
		...
		//crit.add(Restrictions.like("name", "%v%"));
		//crit.add(Restrictions.between("salary",new Double(25000), new Double(50000)));
		//crit.add(Restrictions.eq("name", "vikas"));     
		//crit.add(Restrictions.lt("name", "vikas"));    
		//crit.add(Restrictions.le("name", "vikas"));    
		//crit.add(Restrictions.gt("name", "vikas"));    
		//crit.add(Restrictions.ge("name", "vikas"));    
		...
		//crit.add(Restrictions.and(Restrictions.gt("name", "swati"), Restrictions.lt("name", "vishal")));
		//crit.add(Restrictions.or(Restrictions.gt("name", "swati"), Restrictions.lt("name", "vishal"))); 
